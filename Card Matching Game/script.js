...
const generateGame = () => {
    ...
    const cards = `
        <div class = "board" style = "grid-template-coloums: repeat(${dimensions},auto)">
            ${items.map(item => `
            <div class ="card">
                <div class="card-front"></div>
                <div clas="card-back">${item}</div>
            </div>`).join('')}
        </div> 
    `
...
const startGame = () => {
    ...
    state.loop = setInterval(() =>{
        ...
        selector.moves.innerText = `${states.TotalFlips} moves`;
...
const attachEventListeners = () => {
    document.addEventListener('click',event => {
        ...
        const eventParents = eventTarget.parentElement;

        if(eventTarget.className.includes('card') && !eventParents.className.includes('disabled')){
            startGame();
        }
    })
}
